...
IncumbentCallbackSPP::IncumbentCallbackSPP(IloEnv env, Instance& instS, Solution& solS, const vector<vector<IloIntVar>>& xS, const int HS, const vector<vector<bool>>& NPsS)
    : IloCplex::IncumbentCallbackI(env), inst(instS), sol(solS), x(xS), H(HS), NPs(NPsS)
{
