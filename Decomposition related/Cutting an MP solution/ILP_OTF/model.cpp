...
void M_ILP_OTF(Instance& inst, Solution& sol) {
	...
	while (keepSearching && getCPUTime() - start < inst.timeLimit) {
		...
		cout << " before accessible? = " << sol.perc_sim << endl;
		...
		if (model.get(GRB_IntAttr_Status) == 2) {// a master solution is found
			cout << "Optimal is found..." << endl;
			cout << "accessible? = " << sol.perc_sim << endl;
			...
			cout << "Trying H = " << H + sol.prepacked_height << " actually trying H = " << H << endl; //show the tried height together with the prepacked height
			sol.tLBinc = getCPUTime() - start; //update the time
			forbid_start.clear(); //empty the cuts list
	...
	cout << "Percentage similarity is = " << sol.perc_sim << endl;
	}
	catch (...) {
...
void mycallbackT1::callback() {
	if (where == GRB_CB_MIPSOL) { // if a new incumbent is found
		...
		double timeMIP = round(getDoubleInfo(GRB_CB_RUNTIME) * 1000) / 1000;
		...
		if (status == 1) {
			...
			//add the full cut
			GRBLinExpr cut_this = 0;
			int size_cut = 0;
			set<vector<int>> one_cut;
			vector<vector<int>> cut; //to store the current solution
			for (int j = 0; j < n2; j++) { cut.push_back({ j, getStart[j] }); cut_this += x[getStart[j]][j]; size_cut++; one_cut.insert({ j, getStart[j] }); } // add the full cut

			addLazy(cut_this <= size_cut - 1);
			...
			if (size_deleted) { //only perform this operation if you have deleted something with MIS-LR, otherwise you will do this in the next step anyway!
				...
				if (isValidCut) {
					...
					while (1) {
						...
						int previous_item = item_no;
						while (previous_item == item_no) {
							int item_start = cut[j][1];
							if (inst.NPs[item_no][item_start]) {
								cut_this += x[item_start][item_no]; //add the index in the cut
								one_cut.insert({ item_start, item_no });
							}
							previous_item = item_no;
							j++;
							if (j == cut.size()) break;
							item_no = cut[j][0]; //move to the next item
						}
						RHS++;
						if (j == cut.size()) break;
					}
					//sol.cuts.insert({ one_cut });
					addLazy(cut_this <= RHS - 1);
					atLeast1Cut = true;
				}
			}
