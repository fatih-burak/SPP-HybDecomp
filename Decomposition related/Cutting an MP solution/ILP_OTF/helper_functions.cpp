...
void printInfo(const string& pathAndFileout, const Solution& sol, const string& filein) {
	...
	file << fixed << setprecision(2) << filein << "\t" << sol.opt << "\t" << sol.timeT << "\t" << sol.iLB << "\t" << sol.LB 
		<< "\t" << sol.perc_sim << "\t" << sol.Ncalls << "\t" << sol.Ncuts << "\t" << sol.NfailSl 
		...
		<< "\t" << sol.DmisLR << "\t" << sol.DmisLR_o << "\t" << sol.DmisO << "\t" << sol.DmisR << endl;
	file.close();
}
