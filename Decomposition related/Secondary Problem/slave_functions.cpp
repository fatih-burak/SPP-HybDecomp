...
vector<bool> MIS_O(const vector<vector<int>>& items_slave, const vector<vector<int>>& bins, const int& H, vector<bool>& deleted) {
	...
	for (auto item_info : remainingItems) {
...
int slave(const vector<vector<int>>& items_slave_org, const vector<vector<int>>& bins_org, const int& H, vector<vector<int>>& current_sol, int type) {
	...
	else cp.setParameter(IloCP::TimeLimit, 1200); // actual slave
...
int slave_CP(const vector<vector<int>>& items_slave_org, const vector<vector<int>>& bins_org, const int& H, vector<vector<int>>& current_sol, int type, string instanceName) {
	//type: Preprocessing beginning of the algorithm = 2, MIS call=1, Main Slave call=0 , preprocess slave call = -1;
	...
	// change some settings
	IloCP cp(model);
	cp.setParameter(IloCP::TimeLimit, 1200); // actual slave
	...
	int opt = 0;
	double solTime = cp.getInfo(IloCP::SolveTime);
	if (status == 1) {
		cout << "Slave CP feasible..." << endl;
		opt = 1;
	}
	else if (status == 0) { //time limit reached
		cout << "Time limit in slave CP..." << endl;
		solTime = 1200;
		opt = -1;
	}
	else if (status == 3) { //infeasible
		//Infeasible
		cout << "Slave CP infeasible..." << endl;
	}

	string nameFile = "./_OUTPUT/slave_CP.txt";
	std::ofstream file(nameFile.c_str(), std::ios::out | std::ios::app);
	file << fixed << setprecision(2) << instanceName << "\t" << opt << "\t" << solTime << endl;
	file.close();
...
int slave_ILP(const vector<vector<int>>& items_slave_org, const vector<vector<int>>& bins_org, const int& H, vector<vector<int>>& current_sol, int type, string instanceName) {
	...
	GRBModel model = GRBModel(env);
	// declaration of the variables for the model
	vector<GRBVar> y(n_slave);

	// declaration of the variables for the model
	vector<vector<GRBVar> > b;
	b.resize(n_slave, vector<GRBVar>(n_slave));

	// initizalization of the variables for the model
	for (int i = 0; i < n_slave; i++){
		y[i] = model.addVar(0, H - items_slave[i][1], 0, GRB_CONTINUOUS);
	...
	for (int p = 0; p < bins.size(); p++) {
		if (p > 0 && bins[p] == bins[p - 1]) continue; // Do not check the columns that are the same as one before
		for (int i = 0; i < bins[p].size(); i++) {
			for (int j = i+1; j < bins[p].size(); j++) {
				//if (i == j) continue;
				int ip = bins[p][i];
				int jp = bins[p][j];
				if (ip >= jp) continue;
				int h_ip = items_slave[ip][1];
				int h_jp = items_slave[jp][1];
				model.addConstr(y[ip] + h_ip <= y[jp] + H * (1 - b[ip][jp]));
				model.addConstr(y[jp] + h_jp <= y[ip] + H * (b[ip][jp]));
				model.update();
			}
		}
	}
	...
	model.getEnv().set(GRB_DoubleParam_TimeLimit, 1200); // actual slave

	// optimize	
	model.optimize();

	int opt = 0;
	double solTime = model.get(GRB_DoubleAttr_Runtime);
	int status = -1;
	//cout << model.get(GRB_IntAttr_Status) << endl;
	if (model.get(GRB_IntAttr_Status) == 2) {// feasible
		cout << "Slave ILP feasible..." << endl;
		opt = 1;
		status = 1;
	}
	else if (model.get(GRB_IntAttr_Status) == 9) {
		cout << "Time limit in slave ILP..." << endl;
		solTime = 1200;
		status = 0;
		opt = -1;
	}
	else if (model.get(GRB_IntAttr_Status) == 3) { //Infeasible
		cout << "Slave ILP infeasible..." << endl;
		status = 3;
	}

	string nameFile = "./_OUTPUT/slave_ILP.txt";
	std::ofstream file(nameFile.c_str(), std::ios::out | std::ios::app);
	file << fixed << setprecision(2) << instanceName << "\t" << opt << "\t" << solTime << endl;
	file.close();
