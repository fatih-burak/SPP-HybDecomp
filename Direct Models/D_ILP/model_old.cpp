...
void D_ILP(Instance& inst, Solution& sol) {
	...
	model.getEnv().set(GRB_DoubleParam_TimeLimit, max(inst.timeLimit - sol.timeT, 0.001)); //use the remaining time left if you increase the LB
	model.getEnv().set(GRB_IntParam_Seed, 1);
	//model.getEnv().set(GRB_DoubleParam_BestObjStop, sol.iLB + 0.99);
	...
	model2.optimize();
	sol.contR = model2.get(GRB_DoubleAttr_ObjVal) + sol.prepacked_height;
