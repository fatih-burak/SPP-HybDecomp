...
void CC_CP(Instance& inst, Solution& sol) {
	...
	for (int j = 0; j < n2; j++) {
		...
		// ---- WARM START ----
		int p_warm = get<0>(sol.packing_locations[j]);
		int q_warm = get<1>(sol.packing_locations[j]);
		startSol.setStart(x[j], p_warm);
		startSol.setStart(y[j], q_warm);
	}
	...
	IloIntVar obj(env, 0, IloIntMax); //you can set a LB on z
