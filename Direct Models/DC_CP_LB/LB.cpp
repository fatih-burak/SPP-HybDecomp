...
int LB_CBP_CONF(const Instance& inst, Solution& sol) {
	...
	sol.vLB1 = ceil(model.get(GRB_DoubleAttr_ObjBound) - EPSILON);
	return ceil(model.get(GRB_DoubleAttr_ObjBound) - EPSILON);
...
int LB_RE(const Instance& inst, Solution& sol) {
	...
	try {
		int bound = ceil(model.get(GRB_DoubleAttr_ObjBound));
	...
	sol.vLB2 = ceil(model.get(GRB_DoubleAttr_ObjBound) - EPSILON);
	return ceil(model.get(GRB_DoubleAttr_ObjBound) - EPSILON);
}
...
int LB_PCC_CONF(const Instance& inst, Solution& sol) {
	...
	for (;;) {
		...
		model.getEnv().set(GRB_IntParam_Method, 2);
		model.getEnv().set(GRB_IntParam_MIPFocus, 1);
		model.getEnv().set(GRB_DoubleParam_TimeLimit, 1 - (getCPUTime() - start));
